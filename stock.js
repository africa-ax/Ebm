...
// Applies to Distributor stock, Retailer stock, and Manufacturer Acquired Resources
async function checkAndCleanupStock(docId, data) {
    if (data.quantity <= 0) {
        // Only auto-delete from the 'stock' collection (Distributors, Retailers, and Manufacturer Resources)
        // We do NOT delete from 'products' (Manufacturer's own production) unless specifically requested
...
// ✅ FINAL & CORRECT STOCK UPDATE (TRANSACTION + MERGE)
async function finalizePurchase(purchaseData) {
    /**
     purchaseData MUST contain:
     {
        ownerId,
        productId,
        productName,
    ...
    */
    ...
    if (!purchaseData.ownerId || !purchaseData.productId || purchaseData.quantity <= 0) {
    ...
    await db.runTransaction(async (tx) => {
        ...
        if (!query.empty) {
            ...
            tx.update(doc.ref, {
                quantity: currentQty + purchaseData.quantity,
                lastUpdated: firebase.firestore.FieldValue.serverTimestamp()
            });
        } else {
            // ➕ CREATE NEW STOCK
            const newDoc = stockRef.doc();
            tx.set(newDoc, {
                ...purchaseData,
...
if (stockForm) {
    ...
    tx.update(productRef, { quantity: newQty });
            ...
            // Re-load stock will trigger auto-delete via checkAndCleanupStock if quantity is 0
