...
async function handleSetPrice(e) {
    ...
    try {
        const stockDoc = await db.collection('stock').doc(stockId).get();
        const stock = stockDoc.data();

if (!stock.productId) {
    console.error("❌ STOCK ERROR: productId missing. Cannot sync product.");
    alert("Product linkage error. Please contact admin.");
    return;
}
        ...
        // --- FIX STARTS HERE ---
        // We look for productId OR sku to find the original product
        ...
        if (originalProductId) {
            ...
            if (productDoc.exists) {
                ...
                // Copy ALL essential details for the buyer to see
                updateData.productName = product.productName;
                updateData.unitOfMeasure = product.unitOfMeasure;
                updateData.description = product.description || '';
                updateData.category = product.category || '';
                updateData.imageUrl = product.imageUrl || ''; // Crucial!
                
                // Ensure the stock doc has the productId for future reference
                updateData.productId = originalProductId; 
            }
        } else {
            console.error("CRITICAL: No Product ID found in stock document!");
        }
        // --- FIX ENDS HERE ---

        await db.collection('stock').doc(stockId).update(updateData);
        
        showMessage('Price set & product details synced!', 'success');
        closePriceModal();
        loadStock(); 
    } catch (error) {
...
if (stockForm) {
    stockForm.addEventListener('submit', async (e) => {
        ...
        try {
            // Update the 'products' collection for manufacturers
