...
// Ensures single source of truth and prevents duplicate product entries
async function addOrMergeStock(purchaseData) {
    ...
    if (!existingQuery.empty) {
        ...
        const newQuantity = existingDoc.data().quantity + purchaseData.quantity;
        return await stockRef.doc(existingDoc.id).update({
            quantity: newQuantity,
            lastUpdated: firebase.firestore.FieldValue.serverTimestamp()
        });
    } else {
        // Create new stock entry
        return await stockRef.add({
            ...purchaseData,
            lastUpdated: firebase.firestore.FieldValue.serverTimestamp()
        });
    }
