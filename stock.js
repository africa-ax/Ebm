...
async function handleSetPrice(e) {
    ...
    try {
        // 1. Get current stock data to find the original Product ID
        ...
        // 2. Fetch original product details (Name, Image, Desc) and save them to Stock
        // This fixes the "Error loading product" for Buyers
        if (stockData.productId) {
            ...
            if (productDoc.exists) {
                ...
                updateData.imageUrl = product.imageUrl || ''; // Crucial for buyer view
            }
        }

        // 3. Update the stock document
        await db.collection('stock').doc(stockId).update(updateData);
        
        showMessage('Selling price set & product info updated! Item is now visible to buyers.', 'success');
        closePriceModal();
        loadStock(); 
    } catch (error) {
