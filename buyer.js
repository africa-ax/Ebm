...
async function loadSellerProducts() {
    ...
    // Load stock for distributor/retailer - TEMPORARY WORKAROUND
    ...
    for (const doc of snapshot.docs) {
        ...
        // Filter out items with zero quantity OR zero selling price
        if ((stockData.quantity || 0) <= 0 || (stockData.sellingPrice || 0) <= 0) {
            continue; // Skip items with no stock or no price set
        }
