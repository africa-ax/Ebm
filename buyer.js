...
async function loadSellerProducts() {
    ...
    try {
        ...
        if (selectedSeller.role === 'manufacturer') {
            ...
            snapshot.forEach(doc => {
                ...
                products.push({
                    id: doc.id,
                    productDocId: doc.id, // ADD THIS LINE
                    ...data,
    ...
    // Load stock for distributor/retailer - FIXED VERSION
    ...
    for (const doc of snapshot.docs) {
        ...
        // *** ADD THIS CHECK: Filter out items with zero quantity ***
        if ((stockData.quantity || 0) <= 0) {
            continue; // Skip items with no stock
        }
        
        // *** FIX: Handle cases where product details might be in stock itself ***
        ...
        // Try to get additional details from products collection
        ...
        products.push({
            id: doc.id,
            stockId: doc.id,
            productDocId: stockData.productId, // Save the product document ID
...
function addToCart(product) {
    ...
    if (existing) {
        ...
        cart.push({
            id: product.id,
            productDocId: product.productDocId || product.id, // ADD THIS: Ensure productDocId is included
...
const items = cart.map(item => ({
    productDocId: item.productDocId || item.id, // Use productDocId from cart
