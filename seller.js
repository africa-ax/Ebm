    ...
    try {
        ...
        for (const item of order.items) {
            ...
            if (userRole === 'manufacturer') {
                ...
                // *** CRITICAL FIX: Save the Real Doc ID to pass to buyer ***
...
async function addToRawMaterials(buyerId, buyerData, item, productDetails) {
    // Determine the correct Product Reference ID
    const refId = productDetails.docId || item.productDocId || item.productId;

    const rawMaterialData = {
        productId: refId, // Save the Real Doc ID
        sku: productDetails.productId || item.productId, // Save SKU separately
    ...
    const existingQuery = await db.collection('raw_materials')
        ...
        .where('productId', '==', refId) // Query by Ref ID
        .limit(1)
        .get();
...
async function addToStock(buyerId, buyerData, item, productDetails) {
    // *** CRITICAL FIX: Use the Document ID from productDetails as the productId reference ***
    // This allows the buyer.js to look up the product details correctly later.
    const refId = productDetails.docId || item.productDocId || item.productId;

    const stockData = {
        productId: refId, // This must be the Firestore Doc ID
        sku: productDetails.productId || item.productId, // Optional: Keep SKU for reference
        productName: item.productName,
        ...
        // *** CRITICAL FIX: Ensure Unit of Measure is carried over ***
        unitOfMeasure: productDetails.unitOfMeasure || item.unitOfMeasure || 'pcs',
        category: productDetails.category || 'general',
        description: productDetails.description || '',
        vatType: productDetails.vatType || 'standard', // ADD THIS LINE
        taxCategory: productDetails.taxCategory || 'B', // ADD THIS LINE
