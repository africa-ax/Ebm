...
// In seller.js, update these functions:

// 1. Update setupRealtimeListener() - remove orderBy
function setupRealtimeListener() {
    db.collection('purchaseRequests')
        ...
        .onSnapshot((snapshot) => {
            ...
            loadOrders(); // This will reload orders when there are changes
        }, (error) => {
            console.error('Error in realtime listener:', error);
        });
}

// 2. Update loadOrders() - remove orderBy and sort manually
async function loadOrders() {
    try {
        ...
        for (const doc of snapshot.docs) {
            ...
            console.log('Processing order:', order.id, 'sellerId:', order.sellerId, 'status:', order.status);
        ...
        console.log('Categorized orders:', {
            ...
            invoiced: orders.invoiced.length
        });
...
window.approveOrder = async function(orderId) {
    ...
    try {
        ...
        // Step 2: Generate EBM invoice using ebmMock
        const invoiceData = {
