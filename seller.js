...
async function loadOrders() {
    try {
        ...
        allOrders.forEach(order => {
            ...
            // CHANGE 2 FIX: Treat 'received' (and 'approved') as invoiced
            // This ensures orders moved to 'received' status appear in the Invoiced tab
            if ((status === 'approved' || status === 'received') && order.invoiceId) {
...
function renderInvoicedOrders() {
    ...
    if (orders.invoiced.length === 0) {
        tbody.innerHTML = '<tr><td colspan="7" class="empty-state"><div class="empty-icon">📭</div><p>No invoiced/received orders</p></td></tr>';
        return;
    }
...
window.approveOrder = async function(orderId) {
    ...
    try {
        ...
        await db.collection('sales_invoices').doc(invoice.invoiceId).set({
            ...
            // Save explicit fields for retrieval
            ...
            // CHANGE 1 FIX: Explicitly saving timestamp so formatDate() works
        ...
        await db.collection('purchaseRequests').doc(order.id).update({
            // CHANGE 2 FIX: Status updated to 'received' instead of 'approved'
            status: 'received',
