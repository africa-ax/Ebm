    ...
    try {
        ...
        for (const item of order.items) {
            ...
            if (userRole === 'manufacturer') {
                ...
                // --- FIX STARTS HERE: Try to find product by ID or by FIELD ---
                // 1. Try Document ID first
                let productDoc = await db.collection('products').doc(item.productId).get();
                
                // 2. If not found, look for productId FIELD
                ...
                if (!productDoc.exists) {
                    showMessage(`Product ${item.productName} (ID: ${item.productId}) not found`, 'error');
                    return;
                }
                // --- FIX ENDS HERE ---
                ...
                // Ensure stockId exists (it might be null if ordered directly from manufacturer listing, though usually has stockId)
            ...
            // This satisfies the requirement to check Firestore for taxability
            if (productDetails) {
