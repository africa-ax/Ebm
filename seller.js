...
async function loadOrders() {
    try {
        ...
        for (const doc of snapshot.docs) {
            ...
            console.log('Order:', doc.id, orderData);
            
            // Validate required fields
            if (!orderData.sellerId || !orderData.buyerId) {
                console.warn('⚠️ Order missing required fields:', doc.id);
                continue;
            }
...
function renderPendingOrders() {
    ...
    orders.pending.forEach(order => {
        ...
        row.innerHTML = `
            ...
            <td>
                <span class="items-link" onclick="viewOrderItems('${order.id}')">
...
window.viewOrderItems = function(orderId) {
    ...
    modalBody.innerHTML = `
        ...
        <div class="order-total">
            Total Amount: ${formatCurrency(order.totalAmount)} RWF
        </div>
    `;
