            ...
            modalBody.innerHTML = `
                ...
                <div class="detail-section">
                    ...
                    <div class="detail-grid">
                        ...
                        <div class="detail-item">
                            <div class="detail-label">Address</div>
                            <div class="detail-value">${invoice.buyerAddress || 'N/A'}</div>
                        </div>
                        <div class="detail-item">
                            <div class="detail-label">Phone</div>
                            <div class="detail-value">${invoice.buyerPhone || 'N/A'}</div>
                        </div>
                    </div>
                </div>
    ...
    try {
        ...
        if (invoiceDoc.exists) {
            ...
            printWindow.document.write(`
                ...
                <body>
                    <div class="header">
                        <div class="company-name">${invoice.sellerName || 'Business'}</div>
                    ...
                    <div class="grid">
                        ...
                        <div class="section">
                            ...
                            <div><strong>Address:</strong> ${invoice.buyerAddress || 'N/A'}</div>
                            <div><strong>Phone:</strong> ${invoice.buyerPhone || 'N/A'}</div>
                        </div>
                    </div>
    ...
    try {
        ...
        try {
            ...
            if (buyerDoc.exists) {
                ...
                order.buyerName = buyerData.businessName || buyerData.fullName || 'Customer';
                order.buyerTIN = buyerData.businessTIN || buyerData.phone || 'N/A';
                // FIX: Capture and save full buyer details
                order.buyerAddress = buyerData.businessAddress || 'N/A';
                order.buyerPhone = buyerData.businessPhone || 'N/A';
            }
        } catch (error) {
        ...
        for (const item of order.items) {
            ...
            // FIX: Explicitly assign VAT Type to item for Invoice Generation
            if (productDetails) {
                if (productDetails.vatType === 'exempt' || productDetails.vatType === 'zero') {
                    item.vatRate = 0;
                } else {
                    item.vatRate = 18; // Assuming standard VAT is 18%
                }
                item.taxType = productDetails.taxCategory || 'B';
                item.vatType = productDetails.vatType || 'Standard'; // Explicitly saving string
            }
        }
        ...
        // FIX: Pass all required Seller and Buyer details to the invoice generator
        const invoiceData = {
            // Seller Info (from current user data)
            ...
            // Buyer Info (from fetched order/buyer data)
            buyerName: order.buyerName || 'Customer',
            buyerTIN: order.buyerTIN || 'N/A',
            buyerAddress: order.buyerAddress || 'N/A', // NEWLY ADDED
            buyerPhone: order.buyerPhone || 'N/A',     // NEWLY ADDED
        ...
        // Save the generated invoice to Firestore
        await db.collection('sales_invoices').doc(invoice.invoiceId).set({
            ...
            sellerName: invoiceData.sellerName, // Ensure these are saved in the document
            ...
            buyerAddress: invoiceData.buyerAddress, // Saved to document
            buyerPhone: invoiceData.buyerPhone,     // Saved to document
        ...
        await db.collection('purchaseRequests').doc(order.id).update({
            ...
            buyerRole: order.buyerRole,
            buyerAddress: order.buyerAddress, // Saved to purchase request
            buyerPhone: order.buyerPhone,     // Saved to purchase request
