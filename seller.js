...
window.approveOrder = async function(orderId) {
    ...
    try {
        ...
        for (const item of order.items) {
            // 1. Deduct stock from the Seller
            if (userRole === 'manufacturer') {
                ...
                if (stockSnap.exists) {
                    ...
                    if (newQty <= 0) {
                        // AUTO-DELETE: Remove item from seller stock if quantity hits zero
            ...
            // 2. Transfer Inventory to Buyer (Merging if product exists)
...
async function transferInventoryToBuyer(buyerId, buyerData, item) {
    ...
    if (!existingQuery.empty) {
        // MERGE: Update existing stock quantity
        const existingDoc = existingQuery.docs[0];
        await stockRef.doc(existingDoc.id).update({
            quantity: firebase.firestore.FieldValue.increment(item.quantity),
            lastUpdated: firebase.firestore.FieldValue.serverTimestamp()
        });
        console.log(`Merged ${item.quantity} units into existing stock for buyer.`);
    } else {
        // CREATE: Add new stock entry for the buyer
        const newStockData = {
            ownerId: buyerId,
            ...
            sellingPrice: null, // Buyer must set their own price later
            sellerName: userData.businessName || 'Supplier',
            isResource: buyerData.role === 'manufacturer',
            createdAt: firebase.firestore.FieldValue.serverTimestamp(),
            lastUpdated: firebase.firestore.FieldValue.serverTimestamp()
        };
        await stockRef.add(newStockData);
        console.log(`Created new stock entry for buyer.`);
    }
}
