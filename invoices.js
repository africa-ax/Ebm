...
async function loadPurchaseInvoices() {
    ...
    try {
        ...
        console.log('✅ Purchase invoices loaded:', purchaseInvoices.length);
...
async function loadSalesInvoices() {
    ...
    try {
        // Load from sales_invoices collection (EBM generated)
        const ebmSnapshot = await db.collection('sales_invoices')
        ...
        ebmSnapshot.forEach(doc => {
            ...
            salesInvoices.push({
                ...
                status: invoice.ebmStatus === 'MockIssued' ? 'issued' : (invoice.status || 'pending'),
                createdAt: invoice.createdAt || invoice.dateIssued,
                type: 'sale',
                ebmInvoice: true
            });
        });
        
        // Also load from purchaseRequests where user is seller (for backwards compatibility)
        const ordersSnapshot = await db.collection('purchaseRequests')
            .where('sellerId', '==', currentUser.uid)
        ...
        ordersSnapshot.forEach(doc => {
            ...
            // Only add if not already in EBM invoices
            if (!salesInvoices.find(inv => inv.id === order.invoiceId)) {
                salesInvoices.push({
                    id: doc.id,
                    invoiceId: order.invoiceId || doc.id,
        ...
        console.log('✅ Sales invoices loaded:', salesInvoices.length);
...
window.viewInvoiceDetails = async function(invoiceId, type) {
    const invoice = allTransactions.find(t => t.id === invoiceId || t.invoiceId === invoiceId);
    if (!invoice) return;
    
    // If it's an EBM invoice, load from sales_invoices
    let ebmInvoice = null;
    if (invoice.ebmInvoice && invoice.invoiceId) {
        try {
            const ebmDoc = await db.collection('sales_invoices').doc(invoice.invoiceId).get();
            if (ebmDoc.exists) {
                ebmInvoice = ebmDoc.data();
            }
        } catch (error) {
            console.error('Error loading EBM invoice:', error);
        }
    }
    
    // Get buyer name if not in invoice
    let buyerName = invoice.buyerName || ebmInvoice?.buyer?.name || 'Customer';
    ...
    modalBody.innerHTML = `
        <div class="invoice-detail-section">
            ...
            <div class="invoice-detail-grid">
                <div class="invoice-detail-item">
                    ...
                    <div class="detail-value">${ebmInvoice?.invoiceId || invoice.invoiceId || 'INV-' + invoice.id.substring(0, 8).toUpperCase()}</div>
                </div>
                <div class="invoice-detail-item">
                    <div class="detail-label">Date</div>
                    <div class="detail-value">${formatDate(invoice.createdAt || ebmInvoice?.dateIssued)}</div>
                </div>
                <div class="invoice-detail-item">
                ...
                ${ebmInvoice ? `
                <div class="invoice-detail-item">
                    <div class="detail-label">EBM Status</div>
                    <div class="detail-value">${ebmInvoice.ebmStatus || 'N/A'}</div>
                </div>
                <div class="invoice-detail-item">
                    <div class="detail-label">Fiscal Counter</div>
                    <div class="detail-value">${ebmInvoice.fiscalCounter || 'N/A'}</div>
                </div>
                ` : ''}
            </div>
        </div>
        ...
        <div class="invoice-detail-section">
            ...
            <div class="invoice-detail-grid">
                <div class="invoice-detail-item">
                    ...
                    <div class="detail-value">${type === 'purchase' ? (invoice.sellerName || ebmInvoice?.seller?.name || 'N/A') : buyerName}</div>
                </div>
                ${ebmInvoice ? `
                <div class="invoice-detail-item">
                    <div class="detail-label">TIN</div>
                    <div class="detail-value">${type === 'purchase' ? (ebmInvoice.seller?.tin || 'N/A') : (ebmInvoice.buyer?.tin || 'N/A')}</div>
                </div>
                ` : ''}
            </div>
        </div>
            ...
            <table class="items-table">
                <thead>
                    <tr>
                        ...
                        <th>Unit Price</th>
                        <th>Quantity</th>
                        ${ebmInvoice ? '<th>VAT</th>' : ''}
                        <th style="text-align: right;">Total</th>
                    </tr>
                </thead>
                <tbody>
                    ${(invoice.items || ebmInvoice?.items || []).map(item => `
                        <tr>
                            <td>${item.productName}</td>
                            <td>${formatCurrency(item.unitPrice)} RWF</td>
                            <td>${item.quantity}</td>
                            ${ebmInvoice ? `<td>${formatCurrency(item.vatAmount || 0)} RWF</td>` : ''}
                            <td style="text-align: right;">${formatCurrency(item.total)} RWF</td>
                        </tr>
                    `).join('') || '<tr><td colspan="${ebmInvoice ? '5' : '4'}">No items</td></tr>'}
                </tbody>
            </table>
        </div>
        ...
        ${ebmInvoice ? `
        <div class="invoice-detail-section">
            <div class="invoice-detail-title">EBM Tax Breakdown</div>
            <div class="invoice-detail-grid">
                <div class="invoice-detail-item">
                    <div class="detail-label">Total VAT</div>
                    <div class="detail-value">${formatCurrency(ebmInvoice.totalVAT || 0)} RWF</div>
                </div>
                <div class="invoice-detail-item">
                    <div class="detail-label">Payment Method</div>
                    <div class="detail-value">${ebmInvoice.paymentMethod || 'N/A'}</div>
                </div>
            </div>
        </div>
        ` : ''}
        ...
        <div class="invoice-total">
            Gross Total: ${formatCurrency(invoice.totalAmount || ebmInvoice?.totalAmount)} RWF
        </div>

        ${ebmInvoice?.ebmSignature ? `
        <div class="ebm-section">
            <h4>EBM Verification</h4>
            <pre>Signature: ${ebmInvoice.ebmSignature.substring(0, 50)}...</pre>
        </div>
        ` : ''}
    `;
