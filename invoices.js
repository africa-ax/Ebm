...
// FIXED: Handle index errors properly
async function loadPurchaseInvoices() {
    ...
    try {
        ...
        if (error.code === 'failed-precondition') {
            ...
            // Alternative: Query without orderBy first, then sort in memory
            try {
                const altSnapshot = await db.collection('sales_invoices')
                    .where('buyerId', '==', currentUser.uid)
                    .get();
...
// Show index warning
function showIndexWarning(type) {
    const message = `Firestore index required for ${type} invoices. Data loaded without sorting. ` +
                   `Click <a href="#" onclick="createFirestoreIndex('${type}')">here</a> to create index.`;
    
    const warningEl = document.createElement('div');
    warningEl.className = 'index-warning';
    ...
    const container = type === 'purchase' ? 
        document.getElementById('purchase-tab') : 
        document.getElementById('sales-tab');
    
    if (container) {
        const existingWarning = container.querySelector('.index-warning');
        if (!existingWarning) {
...
// Function to help create Firestore indexes
window.createFirestoreIndex = async function(type) {
    const indexUrl = await generateIndexCreationLink(type);
    if (indexUrl) {
        window.open(indexUrl, '_blank');
        showMessage(`Opening Firestore console to create ${type} index...`, 'info');
    } else {
        showMessage('Unable to generate index URL. Please create index manually in Firebase Console.', 'error');
    }
};

// Generate Firestore console link for index creation
async function generateIndexCreationLink(type) {
    const projectId = firebaseConfig.projectId;
    if (!projectId) return null;
    
    const collectionId = 'sales_invoices';
    const field = type === 'purchase' ? 'buyerId' : 'sellerId';
    
    // Base URL for Firestore console
    const baseUrl = `https://console.firebase.google.com/project/${projectId}/firestore/indexes`;
    
    // Create composite index query
    const indexQuery = `?collectionGroup=${collectionId}&queryScope=COLLECTION&fields=${field}=ASCENDING&fields=timestamp=DESCENDING`;
    
    return baseUrl + indexQuery;
}
...
function showErrorMessage(type, error) {
    ...
    let errorMessage = 'Error loading invoices';
    
    if (error.code === 'failed-precondition') {
        errorMessage = `Firestore index required. Please create a composite index for sales_invoices collection with fields: ${type === 'purchase' ? 'buyerId, timestamp' : 'sellerId, timestamp'}`;
            ...
            <td colspan="7" style="text-align: center; padding: 40px; color: #dc3545;">
                ...
                ${error.code === 'failed-precondition' ? 
                  `<p style="font-size: 12px; margin-top: 10px;">
                    <button onclick="createFirestoreIndex('${type}')" style="padding: 8px 16px; background: #667eea; color: white; border: none; border-radius: 4px; cursor: pointer;">
