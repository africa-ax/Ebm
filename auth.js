...
document.getElementById('business-register-form').addEventListener('submit', async (e) => {
  ...
  try {
    ...
    const tinCheck = await db.collection('users')
      .where('businessTIN', '==', businessTIN)
      .get();
    
    if (!tinCheck.empty) {
      showMessage('This TIN is already registered. Each business must have a unique TIN.', 'error');
      return;
    }
