...
// Click hamburger to return to dashboard view (show front buttons)
hamburgerBtn.addEventListener('click', () => {
    // If sidebar is closing and no section is active, show front buttons
    if (sidebar.classList.contains('active')) {
        const anySectionActive = document.querySelector('.content-section.active');
        if (anySectionActive) {
            // Hide all sections and show front buttons
            document.querySelectorAll('.content-section').forEach(cs => {
                cs.classList.remove('active');
            });
            menuItems.forEach(mi => mi.classList.remove('active'));
            frontButtons.style.display = 'grid';
        }
    }
});
