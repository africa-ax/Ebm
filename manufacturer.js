...
function initializeDashboard(userId) {
    ...
    setupPublicProductForm(userId); // Setup the new Public form
...
function setupNavigation() {
    ...
    // NEW: Back button for public section
...
function setupCreateProductForm(userId) {
    ...
    form.addEventListener('submit', async (e) => {
        ...
        try {
            ...
            showMessage('message-create', 'success', 'Product created successfully! You can now set quantity in Stock section.');
            form.reset();
...
// NEW: Setup Public Product Registry Form
function setupPublicProductForm(userId) {
    const form = document.getElementById('public-product-form');

    form.addEventListener('submit', async (e) => {
        e.preventDefault();

        // Data strictly needed by sellers for invoices
        const publicData = {
            productName: document.getElementById('public-product-name').value.trim(),
            productId: document.getElementById('public-product-id').value.trim(),
            vatType: document.getElementById('public-vat-type').value,
            manufacturerId: userId,
            registeredAt: firebase.firestore.FieldValue.serverTimestamp()
        };

        try {
            // Check if product ID already exists in public registry
            const existingPublicProduct = await db.collection('public_products')
                .where('productId', '==', publicData.productId)
                .get();

            if (!existingPublicProduct.empty) {
                showMessage('message-public', 'error', 'This Product ID is already registered in the public system.');
                return;
            }

            // Store in the 'public_products' collection accessible by sellers
            await db.collection('public_products').add(publicData);

            showMessage('message-public', 'success', 'Product registered to public system! Sellers can now use this for invoicing.');
            form.reset();
            window.scrollTo({ top: 0, behavior: 'smooth' });
        } catch (error) {
            console.error('Error registering public product:', error);
            showMessage('message-public', 'error', 'Failed to register product. Please try again.');
        }
    });
}
