...
window.ebmMock = {
    ...
    generateInvoice: function(data) {
        ...
        // Calculate taxes
        // We pass the items directly, relying on 'vatRate' being populated by seller.js 
        // from Firestore lookup.
        ...
        const invoice = {
            ...
            // Invoice Details
            // Ensure we use the tax info passed in from seller.js
            items: data.items.map(item => ({
                ...item,
                taxType: item.taxType || 'B', // Default to B (18%) if not found
                vatRate: (item.vatRate !== undefined) ? item.vatRate : 18,
                vatAmount: this.calculateVAT(item.total, (item.vatRate !== undefined) ? item.vatRate : 18)
            })),
    ...
    calculateTaxes: function(items) {
        ...
        items.forEach(item => {
            // Respect the vatRate if passed from seller.js, otherwise default to 18
            const vatRate = (item.vatRate !== undefined) ? item.vatRate : 18;
            ...
            if (vatRate === 18) {
                ...
                if (item.taxType === 'A') { // Usually A is Exempt
                    exempt += item.total;
                } else {
                    vat0 += vatAmount; // Zero rated
                }
            } else {
                // Fallback
                exempt += item.total;
